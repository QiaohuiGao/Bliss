...
export function getQuestScopingOverview(
  ...
  return {
    ...
    questions: template.scopingQuestions.map(question => {
...
export function createQuestScopingTools(options: QuestScopingToolOptions): AgentTool[] {
  ...
  const candidateTool: AgentTool = {
    ...
    async execute(input) {
      ...
      if (!question.options.includes(parsed.choice)) {
      ...
      const candidates = tasksForQuest(resolveTree({
        ...
        answers: { ...options.activeAnswers, [parsed.questionKey]: parsed.choice },
  ...
  const proposalTool: AgentTool = {
    ...
    async execute(input, context) {
      ...
      if (!question.options.includes(packet.proposedChoice)) {
